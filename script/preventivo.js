...
function AggiornaPreventivo(selectedData, id) {
    try {
        ...
        if (selectedData.quantity <= 0) removeProduct(id);
        else
            if (_MemPrev.length === 0 || !isInMemPrev(id)) addNewProduct(id, selectedData);
            else updateProduct(id, selectedData);
        if (_MemPrev.length === 0) //ho eliminato ogni elemento dal carrello, disabilito
            DisableQuotePart(true); //true disabilita
        else DisableQuotePart(false);
        refreshTableQuote();
    } catch (e) {
...
function isInMemPrev(id) {
    for ( var idx = 0; idx < _MemPrev.length; idx++)
...
function GetProduct(id) {
    ...
    for ( var idx = 0; idx < prod.length; idx++)
...
function refreshTableQuote() {
    ...
    for ( var ProdIx = 0; ProdIx < _MemPrev.length; ProdIx++) {
        if (ProdIx === 0) {
...
function addTitleSummaryOfQuote() {
    try {
        ...
        indx.className = "colw4";
        indx.colspan = "2";
        indx.appendChild(document.createTextNode("N° Prodott" + (_MemPrev.length === 1 ? "o" : "i")));
...
function GetPrice(quanita, prices) {
    if (!prices || prices.length === 0) throw 'Prodotto SENZA PREZZI contattare l\'assistenza';
...
function controllaMessage(noteField) {
    try {
        ...
        if (btn_sendQuote.disabled) btn_sendQuote.title = "Per abilitare inserire un prodotto nel carrello e validare gli altri campi"
        else btn_sendQuote.title = "";
    } catch (e) {
        alert("gestoreLoad " + e);
    }
}
