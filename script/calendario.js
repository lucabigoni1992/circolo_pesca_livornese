...
function ColorDay(elem, day, isLastDayForType) {
    if (day.count === 0)
        elem.classList.add("current" + day.day, "currentFirst" + day.day, "padd");
    if (day.count > 0 && isLastDayForType)
        elem.classList.add("current" + day.day, "currentLast" + day.day, "padd");
    if (day.count > 0)
        elem.classList.add("current" + day.day, "padd");
}
...
function addOrRemoveClss(eventId, addOrRemove) {
    ...
    while (dt <= curEv.datafine) {
        ...
        if (addOrRemove)
            ...
            currDay.classList.remove("fakehover");
    }
}
